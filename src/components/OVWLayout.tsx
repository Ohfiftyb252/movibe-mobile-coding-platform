    ...
    <div className="flex flex-col gap-2 items-end">
      ...
      <div className="w-48 space-y-1">
        <div className="flex justify-between text-[10px] uppercase tracking-tighter text-ov-primary/70">
          ...
          <span>{player.corruption}%</span>
        </div>
        <Progress value={player.corruption} className="h-1 bg-ov-primary/10" />
      </div>
      {player.debt > 1000 && (
        <div className="fixed top-16 right-4 max-w-xs animate-slide-up z-50">
...
export function OVWLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-ov-dark text-ov-foreground font-mono selection:bg-ov-primary selection:text-black">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-4 backdrop-blur-sm bg-ov-dark/50 border-b border-ov-primary/5">
      ...
      <footer className="fixed bottom-0 left-0 right-0 p-4 text-center text-ov-gray/30 text-[10px] pointer-events-none uppercase tracking-widest">
